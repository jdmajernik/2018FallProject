...
public class W_Bouncer : WeaponMechanics {
    ...
    public override void Fire(Vector3 aimPosition)
    {
        if (base.canShoot)
        {
            ...
            if (isAI)
            {
                bullet.GetComponent<Proj_Bouncer>().good_team = gameObject.GetComponent<AI_Enemy>().currentteam;
            }
            else
            {
                bullet.GetComponent<Proj_Bouncer>().good_team = gameObject.GetComponent<PlayerController>().currentteam;
            }
        }
    }
}
